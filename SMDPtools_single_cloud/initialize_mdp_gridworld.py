    ...
    for c1_i in range(board.board_size[0]):
        for b in range(board.battery_cap):
            ...
            for station_coord in board.station_coords: # the station only allows full tank states
                q_s[station.v].append(tuple(station_coord+[c1_i, 12]))
        ...
        for j in range(board.board_size[1]):
            for c1_i in range(board.board_size[0]):
                if [i, j] not in board.station_coords:
                    q_s[sinks.v].append(tuple([i, j, c1_i, 0])) # when the uav used up its battery outside of the station
                else:
                    for k in range(board.battery_cap):
                        impossible_states.append(tuple([i, j, c1_i, k])) #when the uav is at the station, its power could only be full
