...
class MDP:
    ...
    def __init__(self, gamma = 0.9, epsilon = 0.15, gain = 100.0, tau = 1.0, transition_file=""):
        ...
        self.init_S = []
        self.originS = []
        self.unsafe = []  # unsafe state space, e.g. if state space is [[1,1]], wall is [[1,2], [2,1], [1,0], [0,1]]
    ...
    def AS_reachability(self, F, flag):
    ...
    def set_P(self): # add cloud dynamics to the transition matrix
        ...
        for s in self.S:  # new_S
            if s not in self.unsafe:  # new_wall
                filtered_S.append(s)
        ...
        for state in self.S: # calculates normal transitions when there are still battery left
            ...
            for a in self.A.keys():
                ...
                for ca1 in cloud_acts:
                    ...
                    temp = tuple(list(next_position) + [new_s2] + [s[3] - 1])  # enumerate next states
                    temp_ = tuple(list(next_position) + [new_s2] + [self.full_tank])  # enumerate next states
