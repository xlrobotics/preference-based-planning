        ...
        for i in range(self.board_size[0]):
            for j in range(self.board_size[1]):
                for b in range(self.battery_cap):
                    if dtype=="list":
                        state_set.append([i, j, b]) # each cloud only moves in one axis
                    elif dtype=="tuple":
                        state_set.append(tuple([i, j, b]))  # each cloud only moves in one axis

        for station in self.station_coords: # adding full battery cap states, only at station
            if dtype == "list":
                state_set.append(station+[self.battery_cap])
            elif dtype=="tuple":
                state_set.append(tuple(station + [self.battery_cap]))
        return state_set
