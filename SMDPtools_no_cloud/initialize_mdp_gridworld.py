...
if __name__ == "__main__":
    ...
    for b in range(board.battery_cap):
        if b > 0:
            q_s[A.v].append(tuple(board.goal_coord[0]+[b]))
            q_s[B.v].append(tuple(board.goal_coord[1]+[b]))
            q_s[C.v].append(tuple(board.goal_coord[2]+[b]))

        for station_coord in board.station_coords: # the station only allows full tank states
            q_s[station.v].append(tuple(station_coord+[12]))

    impossible_states = []
    for i in range(board.board_size[0]):
        for j in range(board.board_size[1]):
            if [i, j] not in board.station_coords:
                q_s[sinks.v].append(tuple([i, j, 0])) # when the uav used up its battery outside of the station
            else:
                for k in range(board.battery_cap):
                    impossible_states.append(tuple([i, j, k])) #when the uav is at the station, its power could only be full
